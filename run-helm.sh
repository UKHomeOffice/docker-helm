...
if [[ -z "${KUBE_SERVER}" ]]; then
  versions
  echo "environment variable KUBE_SERVER should be defined as the API server's URL"
  exit 1
fi
...
if [[ -z "${KUBE_CERTIFICATE_AUTHORITY_DATA}" ]]; then
  ...
  KUBE_CERTIFICATE_AUTHORITY_DATA=$(curl -s $KUBE_CERTIFICATE_AUTHORITY | base64)
fi

echo "KUBE_CERTIFICATE_AUTHORITY_DATA: ${KUBE_CERTIFICATE_AUTHORITY_DATA}"
